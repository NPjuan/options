 ...
 * 零依赖，仅使用 Node 内置模块。
 *
 * 数据源：CBOE 官方延迟报价 API（cdn.cboe.com），含完整期权链、
 * 隐含波动率、全套希腊字母、理论价、未平仓量。
 * 该接口无 CORS 头，浏览器无法直连，故由本服务代理并压缩。
 */
...
const server = http.createServer(async (req, res) => {
  ...
  if (u.pathname === '/api/chain') {
    try {
      const data = await handleChain(u.searchParams.get('symbol') || 'AAPL');
  ...
  if (u.pathname === '/api/health') {
    return sendJSON(res, 200, { ok: true, port: PORT, cached: cache.size, time: new Date().toISOString() }, ae);
  }
